...
{				       /* 2 come bets */
  ...
  if (Craps_GetBet(&Table, craps_Come4))
    Bet(&Table, craps_ComeOdds4, 10);

  if (Craps_GetBet(&Table, craps_Come5))
    Bet(&Table, craps_ComeOdds5, 10);
  ...
  if (Craps_GetBet(&Table, craps_Come9))
    Bet(&Table, craps_ComeOdds9, 10);

  if (Craps_GetBet(&Table, craps_Come10))
    Bet(&Table, craps_ComeOdds10, 10);
}
  ...
  char          **argv;
{
  ...
  quiet = 0;
  while ((c = getopt(argc, argv, "12345678w:l:r:q")) != EOF) {
    switch (c) {
    case '1':
      strategy = Strategy1;
      break;
