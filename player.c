...
{				       /* 3 come bets, 6 and 8 always
	...
	if (Craps_GetBet(&Table, craps_PassLineOdds) == 0) {
		...
		case 6:
		case 8:
		    Bet(&Table, craps_PassLineOdds, 10);
		    break;
		default:
		    Bet(&Table, craps_PassLineOdds, 6);
		    break;
	    }
	}
    }
    else {
	Bet(&Table, craps_PassLine, 3);
    }
    if (Craps_GetBet(&Table, craps_Come4))
	Bet(&Table, craps_ComeOdds4, 6);
    if (Craps_GetBet(&Table, craps_Come5))
	Bet(&Table, craps_ComeOdds5, 6);
    if (Craps_GetBet(&Table, craps_Come6))
	Bet(&Table, craps_ComeOdds6, 10);
    if (Craps_GetBet(&Table, craps_Come8))
	Bet(&Table, craps_ComeOdds8, 10);
    if (Craps_GetBet(&Table, craps_Come9))
	Bet(&Table, craps_ComeOdds9, 6);
    if (Craps_GetBet(&Table, craps_Come10))
	Bet(&Table, craps_ComeOdds10, 6);
}
